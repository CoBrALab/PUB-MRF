...
class AWoL_MRF:
    ...
    - Computes patch stats using only the high-confidence voxels
    ...
    def find_lcv(self):
        ...
        if self.lcv.shape[0] == 0: #in this case we just want to return the majority vote
            ...
            for lcv in self.lcv:
                ...
                local_prob = self.probability[:, np.asarray(self.neighbors[lcv])]
    ...
    def get_patch_stats(self):
        ...
        for value in self.candidate_labels[self.lcv[self.lcv_index]]: #get the patch stats for each candidate label
            ...
            weight = np.where(probability > self.threshold, probability - self.threshold, 0.0)[0]
    ...
    def mrf_potentials(self):
        ...
        for value in self.patch_stats.keys(): #compute doubleton and singleton potentials
            ...
            mrf_double = np.dot(0.5 - self.new_probability[np.where(self.label_values==value),n][0], weight)
