...
class AWoL_MRF:
    ...
    def get_mst_sequence(self):
        ...
        seed_coord = np.unravel_index(self.seeds[0], self.labels.shape)
        patch = self.tree.query_ball_point(seed_coord, self.patch_length)
        patch.sort()   
        
        #find all the low-confidence voxels in the patch
        lflat = self.labels.ravel()
        lcvp = np.asarray(patch)[lflat[patch] == -1]
        
        #keep the patch stats for each label that is in the patch
        self.patch_stats = {}
        iflat = self.intensity.ravel()
        for value in self.label_values:
            points = iflat[np.asarray(patch)[lflat[patch] == value]]
