...
class AWoL_MRF:
    ...
    - Does not use the information from newly updated voxels"""
    ...
    def __init__(self, labelimg_list, brainimg, bbox=None, beta=-.2, patch_length=5,
        ...
        self.new_labels = np.copy(self.labels)
    ...
    def mrf_potentials(self):
        ...
        for value in self.patch_stats.keys():
            ...
            mrf_double = self.beta*(2*sum(self.new_labels[n] == value) + sum(self.new_labels[n] == -1) - len(n))
        ...
        self.new_labels[lcv] = new_label #update the label
        ...
        if new_label != 0: #increment the confidence level of all the neighbor low-confidence voxels
            ...
            self.confidence_level[np.searchsorted(self.lcv, neighbors[self.new_labels[neighbors] == -1])] += 1
    ...
    def get_output_image(self):
        ...
        self.labels = self.new_labels.reshape(self.label_shape)        
