...
class AWoL_MRF:
    ...
    - Generalized the minimum information loss walk for multilabel"""
    ...
    def __init__(self, labelimg_list, brainimg, bbox=None, beta=-.2, patch_length=5,
        ...
        for n, img in enumerate(labelimg_list):
            ...
            elif np.asarray(np.unique(label_array) != self.label_values).any(): 
                warn("Labels in {0} not the same as in {1}.".format(img, labelimg_list[0]))
    ...
    def get_patch_stats(self):
        ...
        for value in self.label_values:
            points = iflat[np.asarray(self.patches[self.voxel])[lflat[self.patches[self.voxel]] == value]]
    ...
    def mrf_potentials(self):
        ...
        for value in self.patch_stats.keys():
            ...
            mrf_double = self.beta*(2*sum(lflat[n] == value) + sum(lflat[n] == -1) - len(n))
        ...
        lflat[lcv] = new_label #update the label
        ...
        if new_label != 0: #increment the confidence level of all the neighbor low-confidence voxels
            ...
            self.confidence_level[np.searchsorted(self.lcv, neighbors[self.labels.ravel()[neighbors] == -1])] += 1
        
        del self.neighbors_big[lcv], self.neighbors_small[lcv], self.patch_stats
