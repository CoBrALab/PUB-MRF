...
class AWoL_MRF:
    ...
    def get_patch_stats(self):
        ...
        for value in self.candidate_labels[self.lcv[0]]: #get the patch stats for each candidate label
            ...
            mean = np.sum(intensity * probability) / np.sum(probability) #weighted mean
            std = np.sqrt(np.sum(np.power(intensity - mean, 2) * probability) / np.sum(probability)) #weighted std
