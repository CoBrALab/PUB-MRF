...
class AWoL_MRF:
    ...
    - Uses number of neighbor structural high-confidence voxels to rank seeds
    - Skips the patches that wouldn't update any additional voxel
    - Computes the patch stats using the new information from updated labels
    - Faster implementation to find the neighbors and the patch"""
    
    def __init__(self, labelimg_list, brainimg, bbox=None, beta=-.2, mixing_ratio=10,
                 patch_length=4, same_threshold=True, thresholds=[0.7, 0.7]): #use same defaults as the parser
        ...
        for n, img in enumerate(labelimg_list):
            ...
            if n == 0:
                ...
                warn("Labels in {0} not the same as in {1}.".format(opt.input_labels[n], opt.input_labels[0]))
    ...
    def find_lcv(self):
        ...
        if self.lcv.shape[0] == 0: #in this case we just want to return the majority vote
            ...
            #find the neighborhood of each lcv            
            self.neighbors_small = {}
            self.neighbors_big = {}
            for lcv in self.lcv:
                x, y, z = np.unravel_index(lcv, self.labels.shape) #get coordinates of voxels
                
                small = []
                for coord in [(x-1,y,z),(x+1,y,z),(x,y-1,z),(x,y+1,z),(x,y,z-1),(x,y,z+1)]:
                    small.append(np.ravel_multi_index(coord, self.labels.shape))
                    ...
                    for j in range(y-1, y+2):
                        for k in range(z-1, z+2):
                            if i != x or j != y or k != z:
                                big.append(np.ravel_multi_index((i,j,k), self.labels.shape))
    ...
    def find_seeds(self):
        ...
        for lcv in self.lcv:
            ...
            if n_hcv > self.mixing_ratio: #minimum confidence level of the seeds
        ...
        if len(self.seeds) > 0:
            self.seeds = [seed for (c, seed) in sorted(zip(confidence_level, self.seeds))][::-1] #priority order
            
            self.patches = []            
            for seed in self.seeds:        
                patch = [] #get the list of voxels in the patch   
                x, y, z = np.unravel_index(seed, self.labels.shape)                 
                for i in range(x-self.patch_length, x+self.patch_length+1):
                    for j in range(y-self.patch_length, y+self.patch_length+1):
                        for k in range(z-self.patch_length, z+self.patch_length+1):
                            patch.append(np.ravel_multi_index((i,j,k), self.labels.shape))
    ...
    def get_patch(self):
        ...
        if still_lcv_patch.shape[0] > 0: #if at least 1 lcv wasn't updated yet
            ...
            for value in self.label_values:
                points = iflat[np.asarray(self.patches[0])[nlflat[self.patches[0]] == value]]
                if len(points) > 1: #need at least 2 points to compute standard deviation
