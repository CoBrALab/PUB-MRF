...
class AWoL_MRF:
    ...
    - Doubleton potential is distance-weighted with the 27-voxel neighborhood
    - Patch stats are distance-weighted and include only the high-confidence voxels
    - Output does not depend on the MRF updating sequence"""
    
    def __init__(self, labelimg_list, brainimg, bbox=None, alpha=2.0, beta=2.7, gamma=1.0,
    ...
    def find_lcv(self):
        ...
        if self.lcv.shape[0] == 0: #in this case we just want to return the majority vote
            ...
            for lcv in self.lcv: #find the neighborhood of each lcv
                ...
                self.neighbors[lcv] = neighbors #27-voxel neighborhood
    ...
    def get_patch_stats(self):
        ...
        lcv_coord = np.unravel_index(self.lcv[self.lcv_index], self.label_shape)
            
        for value in self.candidate_labels[self.lcv[self.lcv_index]]: #get the patch stats for each candidate label
            probability = self.probability[np.where(self.label_values == value), patch][0]
            high_confidence = patch[np.where(probability > self.threshold)] #list of local high-confidence voxels
            intensity = self.intensity[high_confidence]
            
            if intensity.shape[0] > 0: #if there is at least 1 local hcv for that label            
                weight = []            
                for hcv in high_confidence: #get the distance weights
                    hcv_coord = np.unravel_index(hcv, self.label_shape)
                    distance = np.linalg.norm(np.subtract(hcv_coord, lcv_coord))
                    weight.append((probability[np.where(patch==hcv)]-self.threshold)*np.exp(-self.gamma*distance))
            
                mean = np.dot(intensity, weight) / np.sum(weight) #weighted mean
                std = np.sqrt(np.dot(np.power(intensity - mean, 2), weight) / np.sum(weight)) #weighted std
...
if __name__ == "__main__":
    ...
    pg.add_argument("-b", "--beta", type=float, default=2.7,
                    help="[default = %(default)s]")
    pg.add_argument("-g", "--gamma", type=float, default=1.0,
                    help="[default = %(default)s]")
    ...
    awolmrf = AWoL_MRF(labelimg_list, brainimg, bbox=np.asarray(bbox), 
                       alpha=opt.alpha, beta=opt.beta, gamma=opt.gamma,
