...
class AWoL_MRF:
    ...
    - Performs autocorrected walks using an iterative minimum spanning tree"""
    ...
    def find_lcv(self):
        ...
        if self.lcv.shape[0] == 0: #in this case we just want to return the majority vote
            ...
            self.updated = []
            self.not_updated = self.lcv.tolist() #list of voxels to update
                
            del self.votes
            
    def next_voxel(self):
        """Find the next voxel to update. The idea is to find the next edge 
        with minimum cost using local intensity gradients. This iterative
        method will automatically subdivide the low-confidence voxels into
        independent subsets and disjoint subsets."""
        
        mincost = np.inf        
        next_voxel = None
        
        #next voxel with minimum cost in the current disjoint subset
        while next_voxel is None and len(self.updated) > 0:
            lcv = self.updated[-1]
            candidates = [voxel for voxel in self.neighbors_big[lcv] if voxel in self.not_updated]
            
            if len(candidates) == 0: #if there isn't any remaining neighbor to update
                self.updated.remove(lcv)
                
            else:
                for candidate in candidates:
                    cost = abs(self.intensity[lcv] - self.intensity[candidate]) #intensity gradient
                    if cost < mincost:
                        mincost = cost
                        next_voxel = candidate
        
        #next disjoint subset
        if next_voxel is None:
            for candidate in self.not_updated: 
                for value in self.candidate_labels[candidate]: #for each structural label
                    cost = -sum(self.labels[self.neighbors_big[candidate]] == value) #confidence level
                    if cost < mincost:
                        mincost = cost
                        next_voxel = candidate #keep the voxel with max confidence level
                    
        self.voxel = next_voxel
        self.updated.append(next_voxel)
        self.not_updated.remove(next_voxel)
    ...
    def mrf_potentials(self):
        ...
        del self.candidate_labels[lcv], self.neighbors_small[lcv]
