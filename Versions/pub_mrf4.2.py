...
class AWoL_MRF:
    ...
    - Considers only the high-confidence voxels for the patch stats"""
    
    def __init__(self, labelimg_list, brainimg, bbox=None, beta=-.2, patch_length=5, threshold=0.7):
        ...
        for n, img in enumerate(labelimg_list):
            ...
            if n == 0:
                ...
                votes = np.zeros((self.label_values.shape[0], label_array.ravel().shape[0]), dtype=np.float16)
        ...
        self.probability = np.array(votes / len(labelimg_list), dtype=np.float16) #get the initial probabilities
    ...
    def find_lcv(self):
        ...
        if self.lcv.shape[0] == 0: #in this case we just want to return the majority vote
            ...
            for lcv in self.lcv:
                ...
                local_prob = self.probability[1:, np.asarray(self.neighbors_big[lcv])]
    ...
    def get_patch_stats(self):
        ...
        self.patch_stats = {}
        patch = np.asarray(self.patches[self.lcv[0]])
            
        for value in self.candidate_labels[self.lcv[0]]: #get the patch stats for each candidate label
            intensity = self.intensity[patch]
            probability = self.probability[np.where(self.label_values == value), patch]
            high_confidence = np.where(probability > self.threshold, probability - self.threshold, 0.0)
            mean = np.sum(intensity * high_confidence) / np.sum(high_confidence) #weighted mean
            std = np.sqrt(np.sum(np.power(intensity-mean,2)*high_confidence)/np.sum(high_confidence)) #weighted std
            
            if std != 0.0: #a standard deviation of 0 doesn't make sense for the singleton computation
                self.patch_stats[value] = [mean, std]
                    
        del self.patches[self.lcv[0]]
    ...
    def mrf_potentials(self):
        ...
        for value in self.patch_stats.keys(): #compute doubleton and singleton potentials
            ...
            mrf_double = self.beta*np.exp(-1)*(2*np.sum(self.probability[np.where(self.label_values==value),n])-len(n))
            mrf_double += self.beta*(2*self.probability[np.where(self.label_values==value),lcv]-1)
            mrf_energy[np.where(self.label_values == value)] = mrf_single + mrf_double

        if len(self.patch_stats.keys()) > 0:        
            self.probability[:, lcv] = np.exp(-mrf_energy) / np.sum(np.exp(-mrf_energy)) #update the probabilities
        
        self.lcv = np.delete(self.lcv, 0) #remove from the lcv array
    ...
    def get_output_image(self):
        ...
        labels = np.zeros(self.label_shape)        
...
if __name__ == "__main__":
    ...
    awolmrf = AWoL_MRF(labelimg_list, brainimg, bbox=np.asarray(bbox), beta=opt.beta, 
                       patch_length=opt.patch_length, threshold=opt.threshold)
