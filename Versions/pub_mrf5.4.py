...
class AWoL_MRF:
    ...
    def get_patch_stats(self):
        ...
        for value in self.candidate_labels[self.lcv[self.lcv_index]]: #get the patch stats for each structural label
            if value != 0:
                intensity = self.intensity[patch]
                probability = self.probability[np.where(self.label_values == value), patch]
                weight = np.where(probability > 0.5, probability - 0.5, 0.0)[0]
    ...
    def mrf_potentials(self):
        ...
        background_single = 0.0
        
        def compute_mrf_single(intensity, mean, std):
            if intensity <= mean: #using the cumulative distribution function
                return -np.log(2*ss.norm.cdf(intensity, loc=mean, scale=std))
            else:
                return -np.log(2*(1-ss.norm.cdf(intensity, loc=mean, scale=std)))
        
        for value in self.patch_stats.keys(): #compute doubleton and singleton potentials
            (mean, std) = self.patch_stats[value]
            distribution_function = ss.norm.cdf(self.intensity[lcv], loc=mean, scale=std)
            if self.intensity[lcv] <= mean: #using the cumulative distribution function
                mrf_single = -np.log(2*distribution_function)
                background_single -= np.log(1 - 2*distribution_function) #handle background
            else:
                mrf_single = -np.log(2*(1 - distribution_function))
                background_single -= np.log(2*distribution_function - 1) #handle background
            
            mrf_prior = -np.log(self.probability[np.where(self.label_values == value), lcv])
            mrf_double = len(n) - 2*np.sum(self.probability[np.where(self.label_values == value), n])
        ...
        background_prior = -np.log(self.probability[0, lcv]) #compute background energy
        background_double = len(n) - 2*np.sum(self.probability[0, n])
        mrf_energy[0] = background_single + self.alpha*background_prior + self.beta*background_double
            
        if self.potential_maps: #update the MRF energy array
