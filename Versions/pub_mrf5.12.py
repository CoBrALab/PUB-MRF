...
class AWoL_MRF:
    ...
    def __init__(self, labelimg_list, brainimg, bbox=None, alpha=2.0, beta=2.7, 
                 patch_length=5, threshold=0.2, potential_maps=False, low_confidence_mask=False):
        ...
        self.low_confidence_mask=bool(low_confidence_mask)
    ...
    def find_lcv(self):
        ...
        if self.low_confidence_mask:
            self.mask_array = np.zeros(self.intensity.shape, dtype=np.uint8)
            self.mask_array[self.lcv] = 1
            self.mask_array = self.mask_array.reshape(self.label_shape)
            
        self.lcv = np.array(()) #temporary
    ...
    def get_output_image(self):
        ...
        if self.bbox is not None:        
            ...
            if self.low_confidence_mask:
                self.mask_array = np.pad(self.mask_array, ((self.bbox[2], self.brainimg.GetDepth() - self.bbox[5]),
                                        (self.bbox[1], self.brainimg.GetHeight() - self.bbox[4]),
                                        (self.bbox[0], self.brainimg.GetWidth() - self.bbox[3])),
                                        "constant", constant_values=0)
                
                self.low_confidence_mask = sitk.GetImageFromArray(self.mask_array)
                self.low_confidence_mask.CopyInformation(self.brainimg)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--low_confidence_mask", type=str, default=None,
                        help="show all the voxels which were updated with PUB-MRF")
    ...
    opt.low_confidence_mask = opt.output_labels.split(".")[0] + "_mask." + ".".join(opt.output_labels.split(".")[1:])
    print(opt.low_confidence_mask)
    
    if opt.low_confidence_mask is not None:
        low_confidence_mask = True
    else:
    ...
    if opt.low_confidence_mask is not None:
        sitk.WriteImage(awolmrf.low_confidence_mask, opt.low_confidence_mask, True)
