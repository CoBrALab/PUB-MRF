...
class AWoL_MRF:
    ...
    - Bounding boxes don't change the fusion labels anymore
    - Corrected a bug where the seeds were excluded from their own patch
    - Now performs the MRF walk on each seed, without any number limitation
    - If possible, output files are compressed"""
    
    def __init__(self, labelimg_list, brainimg, bbox=None, beta=-.2, mixing_ratio=10,
                 patch_length=5, same_threshold=True, thresholds=[0.7, 0.7]): #use same defaults as the parser
        ...
        for i, value in enumerate(self.label_values.tolist()):
            above_threshold = np.where((self.mode[0] == i) & (self.mode[1] >= thresholds[i]*nimg))
            below_threshold = np.where((self.mode[0] == i) & (self.mode[1] < thresholds[i]*nimg))
    ...
    def get_mst_sequence(self):
        ...
        self.seq = [np.argwhere(lcvp == self.seeds[0])[0][0]]
    ...
    def final_labels(self):
        ...
        for value in self.label_values: #assign the majority vote label
            nlflat[still_lcv[0][self.mode[0].ravel()[still_lcv[0]] == value]] = value
