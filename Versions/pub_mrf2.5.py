...
class AWoL_MRF:
    ...
    - Uses tri-mean and median absolute deviation to compute patch stats"""
        ...
        if still_lcv_patch.shape[0] > 0: #if at least 1 lcv wasn't updated yet
            ...
            for value in self.label_values:
                ...
                if len(points) > 1: #need at least 2 points to consider a label in the patch
                    trimean = (np.percentile(points, 20)+2*np.percentile(points, 50)+np.percentile(points, 80))/4.0
                    mad = scale.mad(points, center=trimean)
                    self.patch_stats[value] = [trimean, mad]
    ...
    def mrf_potentials(self):
        ...
        for lcv in self.seq:
            ...
            for value in self.patch_stats.keys():
                (trimean, mad) = self.patch_stats[value]
                mrf_single = (np.log(np.sqrt(2*np.pi)*mad)) + (np.power(iflat[lcv]-trimean,2))/(2*np.power(mad,2))
